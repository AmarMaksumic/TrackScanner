...
filename = directory_path + '/canny_test2/stolacneum.mp4'
file_size = (1280,720) # Assumes 1920x1080 mp4
scale_ratio = 1 # Option to scale to fraction of original size. 

# We want to save the output to a video file
output_filename = 'stolacneum_lanes.mp4'
...
class Lane:
  ...
  def __init__(self, orig_frame):
    ...
    self.roi_points = np.float32([
      (int(3.5*(width/7)),int(8*(height/10))), # Top-left corner
      (int(2.5*(width/7)),int(10*(height/10))), # Bottom-left corner			
      (int(5.5*(width/7)),int(10*(height/10))), # Bottom-right corner
      (int(4.5*(width/7)),int(8*(height/10))) # Top-right corner
    ])
  ...
  def get_line_markings(self, frame=None):
    ...
    _, s_binary = edge.threshold(s_channel, (0, 130))
...
def main():
  ...
  while cap.isOpened():
    ...
    if success:
      ...
      cv2.imshow('l', lane_line_markings)
